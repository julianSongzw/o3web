...
function* factorColorList(action) {
    try {
        // 数据请求参数构造
        let param = {
            reqUrl: dataService.reqUrl.factorColorList,
            reqAuth: '',
            // reqParam:action.payload
        };
        const response = yield call(dataService.postRequest, param);
        ...
        if (response) {
            if (response.data.ret === 0)
                yield put(act.factorColorListQueryFail());
            else {
                yield put(act.factorColorListQuerySuccess(response.data));
            }
        }
    } catch (error) {
        //处理异常
...
export default function* shareSaga() {
    ...
    yield takeEvery(act.FACTOR_COLOR_LIST_QUERY, factorColorList);
