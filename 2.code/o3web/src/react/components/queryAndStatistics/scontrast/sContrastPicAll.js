...
class SContrastPicAll extends React.Component {
    ...
    componentDidMount() {
        let resultDataArr = this.props.objData.factorArr;
        if (resultDataArr.length > 0) {
            for (let i = 0; i < resultDataArr.length; i++) {
                //获取每个返回结果
                let res = this.props.objData.resultData[i];
                //初始化echarts,domid取对比因子的code
                let id = resultDataArr[i];
                ...
                if (res) {
                    let maxArr = this.props.objData.maxArr;
                    let maxValueArr = this.props.objData.maxValueArr;
                    let maxHeightArr = this.props.objData.maxHeightArr;
                    let imgUrl = res.data.imgUrl;
                    let timeArr = [];//时间数组
                    let xData = timeArr.length === 0 ? res.data.firstTimeArr : this.returnDate(timeArr);
    ...
    componentWillReceiveProps(nextProps) {
        debugger;
        //if(nextProps.factorArr!==this.props.factorArr){
        //    this.setState({
        //        factorArr:nextProps.factorArr,//用于提前生成dom,id取对比因子的code
        //    })
        //}
        let resultDataArr = nextProps.objData.resultData;
        if (resultDataArr.length > 0) {
            for (let i = 0; i < resultDataArr.length; i++) {
                //获取每个返回结果
                let res = resultDataArr[i];
                //初始化echarts,domid取对比因子的code
                let id = res.data.factor;
                let myChart = echarts.getInstanceByDom(document.getElementById(id));
                if (myChart) {
                    myChart.dispose();
                }
    ...
    render() {
        ...
        let divArr = [];
        let factorArr = this.props.factorArr;
...
export default SContrastPicAll;
