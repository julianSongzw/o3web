...
namespace LidarFileAndData
{
    ...
    public class ExportPara3w4c
    {
        ...
        private LicelFileIO3w4c File_Lince = new LicelFileIO3w4c();
        ...
        public void Files_DataProcess(int i_index)
        {
            try
            {
                //double fBinWidth = (double)File_Lince.m_DataSet[0]._BinWidth * 2 / 1000;
                double fBinWidth = 0.0075;
                int count_Dataset = File_Lince.m_DataSet[0]._BinVect.Count;
                int joint = 0;

                double[] rawdata355 = new double[count_Dataset];//355
                double[] rawdata532p = new double[count_Dataset];//532P
                double[] rawdata532s = new double[count_Dataset];//532S
                double[] rawdata1064 = new double[count_Dataset];//1064

                double[] rawdata355_filter = new double[count_Dataset];//355
                double[] rawdata532p_filter = new double[count_Dataset];//532P
                double[] rawdata532s_filter = new double[count_Dataset];//532S
                double[] rawdata1064_filter = new double[count_Dataset];//1064

                double[] fSNRatio = new double[count_Dataset];//信噪比

                List<double> listraw0 = new List<double>();
                List<double> listraw1 = new List<double>();
                List<double> listraw2 = new List<double>();
                List<double> listraw3 = new List<double>();

                for (int i = 0; i < count_Dataset; i++)
                {
                    rawdata355[i] = File_Lince.m_DataSet[0]._BinVect[i];
                    rawdata532p[i] = File_Lince.m_DataSet[1]._BinVect[i];
                    rawdata532s[i] = File_Lince.m_DataSet[2]._BinVect[i];
                    rawdata1064[i] = File_Lince.m_DataSet[3]._BinVect[i];

                    listraw0.Add(File_Lince.m_DataSet[0]._BinVect[i]);
                    listraw1.Add(File_Lince.m_DataSet[1]._BinVect[i]);
                    listraw2.Add(File_Lince.m_DataSet[2]._BinVect[i]);
                    listraw3.Add(File_Lince.m_DataSet[3]._BinVect[i]);
                }

                //rawdata0.Add(listraw0);
                //rawdata1.Add(listraw1);
                //rawdata2.Add(listraw2);
                //rawdata3.Add(listraw3);

                //smooothMidlleValue(rawdata355, out rawdata355_filter);
                ...
                #region 信噪比
                GetSNRatio(rawdata532p, fSNRatio, fBinWidth, count_Dataset);
                #endregion
                ...
                GetPRR(rawdata532p, single_PRR0, count_Dataset, fBinWidth, 15, 16, 0.15, 0.25);
                ...
                GetPRR(rawdata355, single_PRR1, count_Dataset, fBinWidth, 15, 16, 0.15, 0.25);
                ...
                GetExtinction(rawdata532p, single_fOutBinsExtinction, count_Dataset, fBinWidth, 532, 90, 15, 16, 0.15, 0.25);
                ...
                GetExtinction(rawdata355_filter, single_fOutBinsExtinction355, count_Dataset, fBinWidth, 355, 90, 15, 16, 0.15, 0.25);
                templist = NanDelete(single_fOutBinsExtinction355);
                datapara.fOutBinsExtinction355.Add(templist);

                templist = new List<double>();
                double[] single_fOutBinsExtinction1064 = new double[count_Dataset];
                GetExtinction(rawdata1064, single_fOutBinsExtinction1064, count_Dataset, fBinWidth, 532, 90, 15, 16, 0.15, 0.25);
                templist = NanDelete(single_fOutBinsExtinction1064);
                datapara.fOutBinsExtinction1064.Add(templist);
                #endregion
                ...
                GetPRR(rawdata532p, p_prr, count_Dataset, fBinWidth, 15, 16, 0.15, 0.25);//0通道PRR
                GetPRR(rawdata532s, s_prr, count_Dataset, fBinWidth, 15, 16, 0.15, 0.25);//2通道PRR
                double[] single_fDepolRatio = new double[count_Dataset];//单次退偏数组
                GetDepolRatio(p_prr, s_prr, single_fDepolRatio, count_Dataset, 1);//增益暂定1
                templist = NanDelete(single_fDepolRatio);

                for (int k = 0; k < count_Dataset; k++)
                {
                    if (fSNRatio[k] < 2)
                        templist[k] = 0;
                }
                ...
                GetCloudBaseTh(rawdata532p, fBinWidth, 10, 12, ref cloud_k, single_fcloudbase, single_fcloudpeak, single_fcloudtop, count_Dataset, 0, 0.2);
        ...
        public void Files_Load(string filepath, int axsiTop)
        {
            try
            {
                ...
                {
                    File_Lince = new LicelFileIO3w4c();
        ...
        public void Files_Load(string filepath, int axsiTop, DateTime start, DateTime end)
        {
            ...
            else
            {
                try
                {
                    ...
                    DirectoryInfo nowdir = new DirectoryInfo(filepath);
                    FileInfo[] lfiles = nowdir.GetFiles();
                    IEnumerable<FileInfo> fileExist = lfiles.Where(p => p.LastWriteTime >= start && p.LastWriteTime <= end);
                    List<FileInfo> newlfiles = fileExist.ToList();

                    for (int j = 0; j < newlfiles.Count; j++)
                    {
                        datapara.filenamelist.Add(newlfiles[j].FullName);
                        string name = newlfiles[j].Name.Replace("HF", ""); 
                        dt = DateTime.ParseExact(name, "yyyyMMddHH.mmss", System.Globalization.CultureInfo.CurrentCulture);
                        datapara.Dic_DataBase.Add(new Tuple<DateTime, int, int>(dt, j++, database_km));
                        datapara.addedfile_count++;
                    }
                    File_Lince = null;
