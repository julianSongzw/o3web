...
module.exports = function(d_tp) {
  d_tp.settings.acls = acl;
  /*查询*/
  d_tp.list = function (data, cb) {
    var filter = {
      order: 'time_cj ASC',
      include:'device',
      where: {}
    ...
    d_tp.find(filter, function (err, rs) {
      if (err) {
        cb(null, errColMsg);
        errLog.error('退偏基础数据查询失败：' + err);
        return false;
      }
      d_tp.count(filter.where, function (err, count) {
        if (err) {
          cb(null, errColMsg);
          errLog.error('退偏基础数据查询失败：' + err);
          return false;
  ...
  d_tp.remoteMethod('list', {
    description: '退偏基础数据查询',
