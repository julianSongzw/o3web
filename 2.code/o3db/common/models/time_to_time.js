...
module.exports = function (time_to_time) {
  time_to_time.settings.acls = acl;
  /*时序图图结构查询*/
  time_to_time.list = function (data, cb) {
    co(function* () {
      let filter = {
        order: 'time_cj ASC',
        where: {}
      ...
      // time_cj_start = Date.parse(new Date(hourFormat(time_cj_start)))+start_index*config.sourceInterval*60*1000;
      let time_cj_end = parseInt((Number(data.time_cj_end) + 1000) / 1000) * 1000;
  ...
  time_to_time.remoteMethod('list', {
    description: '时序图数据格式',
...
function pcolor(param, callback) {
  ...
  param.time_to_time.app.models[table].find(param.filter, function (err, rs) {
    if (err) {
      callback(null, []);
      errLog.error('时序图查询异常：' + err);
      return false;
    }
    for (let j = 0; j < param.h_value.length; j++) {
      ...
      for (let y = 0; y < time_cj.length; y ++) {
        let rs_index;
        for (let i = 0; i < rs.length; i++) {
          let rsobjet = rs[i];
          if (time_cj[y][1] === rsobjet.time_cj) {
            rs_index = i;
          }
        }
        let hour = new Date(time_cj[y][1]);
        let h = hour.getHours();
        if (rs_index || rs_index === 0) {
          point.push([time_cj[y][0], h, rs[rs_index][h_j] === null ? 0 : rs[rs_index][h_j]]);
        } else {
          point.push([time_cj[y][0], h, 0]);
        }
      }
      data[h_value] = point
    }
