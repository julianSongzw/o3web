...
module.exports = function(d_o3_1h) {
  /*查询*/
  d_o3_1h.list = function (data, cb) {
    var filter = {
      order: 'time_cj ASC',
      where: {}
    };
    ...
    d_o3_1h.find(filter, function (err, rs) {
      if (err) {
        cb(null, errColMsg);
        errLog.error('臭氧1h数据查询失败：' + err);
        return false;
      }
      d_o3_1h.count(filter.where, function (err, count) {
        if (err) {
          cb(null, errColMsg);
          errLog.error('臭氧1h数据查询失败：' + err);
          return false;
  ...
  d_o3_1h.remoteMethod('list', {
    description: '臭氧1h数据查询',
