...
module.exports = function(d_o3_m) {
  /*查询*/
  d_o3_m.list = function (data, cb) {
    var filter = {
      order: 'time_cj ASC',
      where: {}
    };
    ...
    d_o3_m.find(filter, function (err, rs) {
      if (err) {
        cb(null, errColMsg);
        errLog.error('臭氧月平均数据查询失败：' + err);
        return false;
      }
      d_o3_m.count(filter.where, function (err, count) {
        if (err) {
          cb(null, errColMsg);
          errLog.error('臭氧月平均数据查询失败：' + err);
          return false;
  ...
  d_o3_m.remoteMethod('list', {
    description: '臭氧月平均数据查询',
