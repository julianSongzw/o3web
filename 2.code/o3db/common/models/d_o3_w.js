...
module.exports = function(d_o3_w) {
  /*查询*/
  d_o3_w.list = function (data, cb) {
    var filter = {
      order: 'time_cj ASC',
      where: {}
    };
    ...
    d_o3_w.find(filter, function (err, rs) {
      if (err) {
        cb(null, errColMsg);
        errLog.error('臭氧周平均数据查询失败：' + err);
        return false;
      }
      d_o3_w.count(filter.where, function (err, count) {
        if (err) {
          cb(null, errColMsg);
          errLog.error('臭氧周平均数据查询失败：' + err);
          return false;
  ...
  d_o3_w.remoteMethod('list', {
    description: '臭氧周平均数据查询',
