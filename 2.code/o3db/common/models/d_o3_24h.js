...
module.exports = function(d_o3_24h) {
  /*查询*/
  d_o3_24h.list = function (data, cb) {
    var filter = {
      order: 'time_cj ASC',
      where: {}
    };
    ...
    d_o3_24h.find(filter, function (err, rs) {
      if (err) {
        cb(null, errColMsg);
        errLog.error('臭氧24h数据查询失败：' + err);
        return false;
      }
      d_o3_24h.count(filter.where, function (err, count) {
        if (err) {
          cb(null, errColMsg);
          errLog.error('臭氧24h数据查询失败：' + err);
          return false;
  ...
  d_o3_24h.remoteMethod('list', {
    description: '臭氧24h数据查询',
